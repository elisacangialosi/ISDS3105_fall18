...
ggplot(data = selfiesCasualties) +
  geom_bar(aes(x = class, y = ..prop.., fill = gender, group = gender))
...
#' 3.
ggplot(data = dt) +
  geom_bar(aes(x = College, y = ..prop.., fill = Sex))
```
...
ggplot(data =dt) +
 geom_bar(aes(x = Ethnicity, fill = Sex)) +   
  facet_wrap(~ College, scales = 'fixed') + #it will prevent the reshaping of the bars
...
ggplot(data = dt) +
  geom_bar(aes(College, fill = Sex)) +
  scale_fill_manual(values  = c("deeppink", "aquamarine1"))
...
ggplot(data = dt) +
  ...
  scale_fill_discrete(name= 'Gender', labels = c('Female' , 'Male'), position ='bottom' )
